...
def daily_settings_kb(enabled: bool):
    buttons = [
        [InlineKeyboardButton(text="🕘 Изменить время", callback_data="daily_reminder_set_time")]
    ]
    if enabled:
        buttons.append([InlineKeyboardButton(text="🔕 Выключить ежедневное напоминание", callback_data="daily_reminder_disable")])
    else:
        buttons.append([InlineKeyboardButton(text="🔔 Включить ежедневное напоминание", callback_data="daily_reminder_enable")])
...
async def daily_reminder_set_time(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text(
        "Введите время ежедневного напоминания в формате HH:MM (например 09:30).",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="⬅️ Назад", callback_data="daily_reminder_settings")]
        ])
    )
    await cb.answer()
...
async def daily_reminder_time_msg(message: Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer("✅ Время обновлено. Ежедневное напоминание включено.", reply_markup=main_menu(message.from_user.id))
